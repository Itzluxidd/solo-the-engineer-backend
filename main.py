...
def separate_stems():
    """
    Simple stem separation placeholder.

    For now, we just return the same audio as both 'vocals' and 'instrumental'
    so the feature works end-to-end without heavy ML dependencies.
    """
    try:
        ...
        # Save two copies with different labels
...
def platform_check():
    ...
    try:
        ...
        if y.ndim == 1:
            y = np.array([y, y])  # ensure stereo shape [channels, samples]
